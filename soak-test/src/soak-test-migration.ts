...
const urlBasePathGenerateV1 = getVersionedBaseUrl(
  config.URL_BASE_PATH_GENERATE_CONTRACT!!,
  "v1"
);
...
export default function () {
    ...
    JSON.stringify(requestPm),
    {
      headers: {
        ...
        "Ocp-Apim-Subscription-Key":
          config.API_SUBSCRIPTION_KEY_GENERATE_CONTRACT!!,
      },
      timeout: "10s",
  ...
  if (Math.random() <= config.MIGRATION_DELETE_RATIO) {
    // delete wallet
    ...
    check(
      response,
      {
        "Response from PUT /migrations/updateDetails was 200": (r) =>
          r.status == 200,
      },
...
function generateUpdateRequest(contractId: string): WalletPmCardDetailsRequest {
  ...
  const formattedMonth =
    monthAsString.length == 1 ? `0${monthAsString}` : monthAsString;
  return {
